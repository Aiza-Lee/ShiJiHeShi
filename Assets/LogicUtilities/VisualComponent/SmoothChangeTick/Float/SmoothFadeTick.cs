...
namespace LogicUtilities
{
	...
	/// 内部不做alpha合法性clamp
	/// </summary>
	public class SmoothFadeTick : ISmoothChangeTick<float>
	{
		...
		protected override void DealPosition() {
			...
			_material.SetFloat(_valueName, Mathf.Clamp01(_material.GetFloat(_valueName) + _distance * (cgDt.SpeedCurve.Evaluate(_elapsedTime / cgDt.ChangeTime) * deltaTime / (cgDt.ChangeTime * cgDt.Integral))));
